...
<jsp:include page="../header.jsp"><jsp:param name="errorMsg" value="${error}"/></jsp:include>
    ...
    <main>
        <h2>Contenu de la facture : ${bill.billNumber} du client ${customer.name}</h2>
        <table>
            <thead>
            <tr>
        ...
        <table>
            ...
            <c:forEach var="productBill" items="${bill.productsBillsList}">
                <tr>
                    ...
                    <td><form action="/product/update" method="get"><input type="hidden" name="productId" value="${product.id}"><button>Modifier</button></form></td>
                    <td><form action="/bill/product/remove" method="post"><input type="hidden" name="billId" value="${bill.id}"><input type="hidden" name="productId" value="${product.id}"><button>Supprimer</button></form></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
